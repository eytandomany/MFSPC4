...
/* -------------------------------------------------------------------- 
   Distance_AC:
   In order to detect anti-correlations as well as correlations, this function
   returns X^2+Y^2-2cos(2t)|X||Y|, where t in the angle between X and Y. 
   -------------------------------------------------------------------- */
double Distance_AC(int D, double *X, double *Y) {
  int d;
  double nx=0, ny=0, xy=0, cos2t;
  for (d = 0; d <  D; d++){
    xy += X[d] * Y[d];
    nx += X[d] * X[d];
    ny += Y[d] * Y[d];
  }
  cos2t = 2*xy*xy/(nx*ny) - 1; /* cos(2t) = 2cos(t)^2 - 1 */
  return sqrt( nx + ny - 2*cos2t*sqrt(nx*ny) );
} 
...
double Distance(int D, double *X, double *Y) {
  ...
  double dist = 0.0;
  double diff;
  static int anticorr = -1;
  if( anticorr<0 )
    anticorr = GetParam( "AntiCorrelation" )? 1 : 0;
 
  if( anticorr )
    return Distance_AC( D, X, Y );
...
RaggedArray EdgeDistance( int D, UIRaggedArray NK, double **X ){
     ...
     for(k = 0; k<M.c[i]; k++ ) {
         M.p[i][k] = D? ( metric? Distance(D,X[i],X[NK.p[i][k]])
			  : Distance_Linf(D,X[i],X[NK.p[i][k]]) )
	                : X[i][NK.p[i][k]];
         if( M.p[i][k] < INF ) chd+=(double)M.p[i][k], nedges++;
      }
   }
   nedges /= 2;
   ...
   FSetParam( "CharDist", (float)(chd / (2.0 * (double)nedges)) );
   return M;
}
