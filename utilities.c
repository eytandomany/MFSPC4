...
double* InitDVector( long n ) {
   double* p;
   p = malloc( (size_t) (n*sizeof(double)) );
   assure( p, "allocation error" );
   return p;
}
...
int findcmp(const void *i, const void *j) {
   static float h;
   h = ((findex*)i)->p[((findex*)i)->i] - ((findex*)j)->p[((findex*)j)->i];
   if( h>((float)0.0) ) return 1;
   if( h<((float)0.0) ) return -1;
   return 0;
}
...
/**
   ...
   utilities.c
**/
void FSortIndex( int n, float* a, unsigned int* j ) {
   findex *indx;
   int k;
   indx = (findex *)malloc( n*sizeof(findex) );
   for( k=0; k<n; k++){
      indx[k].p = a;
      indx[k].i = k;
  }
  qsort(indx,n,sizeof(findex),findcmp);
  for( k=0; k<n; k++)
     j[k] = indx[k].i;
  free(indx);
}
