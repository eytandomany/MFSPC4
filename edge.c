...
{ return (int)(*((unsigned int*)i)) - (int)(*((unsigned int*)j)); }
...
UIRaggedArray knn( int N, int D, double** X, char **mst_edges ) {
     ...
     nk.p = (unsigned int**)calloc(N,sizeof(unsigned int*));
     if (gomstree) {
	    occ_mst = (unsigned int*)calloc(N,sizeof(unsigned int));
	 }
	 ...
	 for(j=0;j<(N-1);j++)
       ...
       for (j=0;j<N;j++)
		   nk.c[i]+=(occ[j]>0 || occ_mst[j]>0);
       nk.p[i] = (unsigned int*)calloc(nk.c[i],sizeof(unsigned int));
       for (k=0,j=0;j<N;j++){
	 if (occ[j] || occ_mst[j]) nk.p[i][k++]=j;
	 if (occ_mst[j]==1 && occ[j]==0)
		 mst_edges[i][j]=1;
	 }
	   }
	  else
...
UIRaggedArray MstEdges(UIRaggedArray NK,char **mst_edges)
{
	...
	UIRaggedArray nk_mst;
     ...
     nk_mst.n = NK.n;
     nk_mst.c = (unsigned int*)calloc(NK.n,sizeof(unsigned int));
     nk_mst.p = (unsigned int**)calloc(NK.n,sizeof(unsigned int*));
	 for (i=0 ; i<NK.n ; i++){
		nk_mst.p[i] = (unsigned int*)calloc(NK.c[i],sizeof(unsigned int));
		for (k=0 ; k < NK.c[i] ; k++)
			if (mst_edges)
				nk_mst.p[i][k] = (mst_edges[i][ NK.p[i][k] ])? NK.n : NK.p[i][k];
			else
				nk_mst.p[i][k] = NK.p[i][k];
	 }
	 return nk_mst;
}
...
UIRaggedArray nknn( int N, int D, double** X ) {
     ...
     double  *dist;		/* distances      */
     int    **MNV;	/* Nearest neighbours array */
     int    **MNV_um;	/* Nearest neighbours array */
     unsigned int    *um_indx;	/* auxiliar */
     unsigned int    *indx;	/* auxiliar */
     UIRaggedArray   nk;        /* returned array */
     unsigned int **edg;        /* edges of mst */
     unsigned int *occ;
     ...
     dist = calloc(N,sizeof(double));
     MNV = InitIMatrix(N,K);
     MNV_um = InitIMatrix(N,N-K);
     um_indx = InitUIVector(N);
     indx = InitUIVector(N);
     metric = ( GetParam( "InfMetric" ) == NULL );
     similiarity=(GetParam("DataIsInteraction")!=NULL);
     ...
     for (i=0;i<N;i++) {
       for(j=0;j<K;j++) {
		...
		if (K==k) {
			MNV_um[i][um_indx[i]] = MNV[i][j];
			um_indx[i]++;
			MNV_um[MNV[i][j]][um_indx[MNV[i][j]]] = i;
			um_indx[MNV[i][j]]++;
		}
		MNV[i][j]-=(MNV[i][j]+1)*(K==k); /* If the candidate is rejected */
	                                  /* its name is replaced by (-1). */
