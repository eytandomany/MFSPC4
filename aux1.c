...
{
   int  nb, i;
   static int  Qblocks[100]; /* max potts spins is 100 */
   /* go over 0 to Q-1 : remember their blocks in A*/
   for( i=0; i < Q; i++ )
     Qblocks[i]=Block[i];
   for (nb = 0; nb < NBlk; nb++)
      NewSpinValue[nb] = IRAND(Q);
   /* go over A and change NewSpinValue to i */
   for(i =0; i < Q; i++)
     NewSpinValue[Qblocks[i]]=i;
   if (StateN==NULL) {
     for(i = 0; i < N; i++)
		...
		unsigned int* Stack )
{
   ...
   for(i = 0; i < N; i++)
      if (Block[i] == UINT_MAX && thClusterSize[ thBlock[i] ]>=min_sz){        /* point i was not labeled yet */
        ns = 0;
        nblock ++;
        Block[i] = nblock;
        ClusterSize[nblock] = 1;
...
void DefaultParam()
{
   ...
   ISetParam( "DirectedGrowthMinSize",	1  );
}
